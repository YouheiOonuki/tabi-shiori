...
(function () {
  ...
  function renderPrintHelp() {
    $('print-help').innerHTML = mode === 'booklet'
      ? '印刷の画面で、用紙「A4」・向き「横」・<strong>両面印刷（短辺とじ）</strong>・倍率 100%（余白なし）を選びます。紙を順に重ねて真ん中で折ると、1 ページ目から順に読める A5 の冊子になります。PDF にするときは、プリンターの選択で「PDF に保存」（Android は「PDF として保存」）を選びます。'
      : '印刷の画面で、用紙「A4」・向き「縦」・倍率 100% を選びます。PDF にするときは、プリンターの選択で「PDF に保存」（Android は「PDF として保存」）を選びます。';
  }
